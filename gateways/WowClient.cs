...
public class WowClient(IHttpClientFactory httpClientFactory)
{
    ...
    public async Task<RawAuctionsResponse> GetAuctions(AuctionHouse auctionHouse)
    {
        var httpClient = GetHttpClient();

        var response =
            await httpClient.GetAsync($"data/wow/connected-realm/5832/auctions/{(int)auctionHouse}?namespace={DYNAMIC_NAMESPACE}");
